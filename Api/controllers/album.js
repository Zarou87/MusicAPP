...
function updateAlbumById(id, update, res) {
    Album.findByIdAndUpdate(id, update, (err, albumUpdate) => {
        if (!err) {
            if (albumUpdate) {
                res.status(200).send({ message: `The album '${albumUpdate.title}' has been updated successfully` });
            } else {
    ...
    Song.find({ album: album?._id }).remove((err, songRemoved) => {
        if (err) {
            ...
            if (!songRemoved) {
                ...
                res.status(200).send({ message: `The album '${album.title}' has been deleted successfully` });
            }
        }
    });
}
...
function getAlbums(req, res) {
    ...
    if (!artistId) {
        ...
        Album.paginate({ artist: artistId }, options, (err, albums) => solveAlbums(res, err, albums));
    }
}
